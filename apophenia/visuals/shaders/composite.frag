#version 330

// COMPOSITOR — final post-FX stage running over the shader-engine FBO.
//
// Phase-10 reshape: this used to also blend an SDXL-Turbo AI texture,
// but the AI tier was repurposed (it now writes shader *parameters*
// rather than producing image bytes), so the composite stage is
// shader-only. The post-FX chain is unchanged from phase 7:
//
//   1. KALEIDOSCOPE (radial mirror) when segments > 1
//   2. GLITCH (animated horizontal block displacement)
//   3. sample shader_tex
//   4. CHROMATIC aberration: re-sample R/B at small UV offsets
//   5. saturation
//
// Inputs:
//   u_shader_tex             - offscreen FBO from ShaderEngine
//   u_glitch                 - state.fx.glitch ∈ [0, 1]
//   u_chromatic              - state.fx.chromatic ∈ [0, 1]
//   u_kaleidoscope_segments  - state.fx.kaleidoscope ∈ [1, 12]; 1 = off
//   u_saturation             - state.palette.saturation ∈ [0, 2]
//   u_time                   - render-frame wallclock seconds; drives the
//                              per-row hash so glitch animates over time

uniform sampler2D u_shader_tex;
uniform float     u_glitch;
uniform float     u_chromatic;
uniform int       u_kaleidoscope_segments;
uniform float     u_saturation;
uniform float     u_time;

in  vec2 v_uv;
out vec4 fragColor;

float hash11(float p) {
    p = fract(p * 0.1031);
    p *= p + 33.33;
    p *= p + p;
    return fract(p);
}

vec2 kaleidoscope(vec2 uv, int segments) {
    if (segments <= 1) return uv;
    vec2 p = uv - 0.5;
    float r = length(p);
    float a = atan(p.y, p.x);
    float wedge = 6.283185307179586 / float(segments);
    a = mod(a, wedge);
    a = abs(a - wedge * 0.5);
    return vec2(cos(a), sin(a)) * r + 0.5;
}

vec2 glitch_uv(vec2 uv, float intensity, float t) {
    if (intensity <= 0.0) return uv;
    float row = floor(uv.y * 40.0);
    float h = hash11(row + floor(t * 8.0) * 0.137);
    float displaced = (h > 0.85) ? (h * 2.0 - 1.0) : 0.0;
    return vec2(uv.x + displaced * intensity * 0.06, uv.y);
}

vec3 sample_shader(vec2 uv) {
    return texture(u_shader_tex, uv).rgb;
}

vec3 apply_saturation(vec3 rgb, float s) {
    float luma = dot(rgb, vec3(0.299, 0.587, 0.114));
    return mix(vec3(luma), rgb, s);
}

void main() {
    vec2 uv = kaleidoscope(v_uv, u_kaleidoscope_segments);
    uv = glitch_uv(uv, u_glitch, u_time);

    // Chromatic aberration: re-sample R / B at horizontal offsets.
    float aberr = u_chromatic * 0.012;
    vec3 base   = sample_shader(uv);
    vec3 base_r = sample_shader(uv + vec2(-aberr, 0.0));
    vec3 base_b = sample_shader(uv + vec2( aberr, 0.0));
    vec3 mixed  = vec3(base_r.r, base.g, base_b.b);

    mixed = apply_saturation(mixed, u_saturation);
    fragColor = vec4(mixed, 1.0);
}
